...
<template>
  <div class="game-play" @click="onGlobalClick">
    ...
    <div class="game-main" ref="mainRef">
      ...
      <template v-else-if="store.currentNode">
        <div class="content-wrapper">
          ...
          <div class="narrative-box" v-if="displayedText">
            <div class="narrative-text" v-html="displayedText" />
          </div>

          <!-- Dialogue lines (chat bubbles) -->
          <div v-if="dialogLines.length" class="dialogue-area">
            <div v-for="(dl, i) in visibleLines" :key="'dl'+i" class="chat-row" :class="{ me: i % 2 === 1 }">
              <div class="chat-avatar">{{ dl.speaker[0] }}</div>
              <div class="chat-bubble">
                <div class="chat-name">{{ dl.speaker }}</div>
                <div class="chat-text" v-html="md2html(dl.text)" />
              </div>
            </div>
            <div v-if="dialogIdx < dialogLines.length - 1" class="continue-hint">
              <span class="arrow">▼</span>
            </div>
          </div>

          <!-- Transitions inline (no box, typewriter-style) -->
          <div v-if="store.transitionText && !isTyping" class="trans-inline">
            <div class="trans-text" v-html="md2html(store.transitionText)" />
          </div>
          ...
          <div v-if="showChoices" class="choice-area">
            <button v-for="c in store.choices" :key="c.id"
              class="choice-btn"
              :class="{ warp: c.source === 'special_warp', chosen: chosenIds.has(c.id), 'scene-trans': c.next_node_id !== store.currentNode?.id }"
          ...
          <!-- Panels -->
          <div v-if="showBP" class="panel"><div v-if="!store.currentState||store.currentState.inventory.length===0" class="p-empty">empty</div><div v-for="(it,i) in (store.currentState?.inventory??[])" :key="i" class="p-row"><span class="p-name">{{ it.name }}</span><button v-if="canDiscard(it)" @click="discardItem(i)" class="p-del">drop</button></div><button class="p-close" @click="showBP=false">close</button></div>
          <div v-if="showLoad" class="panel"><div v-if="saveList.length===0" class="p-empty">no saves</div><div v-for="s in saveList" :key="s.id" class="p-row"><span class="p-name">{{ s.save_name||s.id }}</span><span class="p-meta">{{ s.current_node_id }}·c{{ s.cycle_count }}</span><button @click="doLoad(s.id)">load</button><button @click="doDelete(s.id)" class="p-del">del</button></div><button class="p-close" @click="showLoad=false">close</button></div>
        </div>
      </template>

      <div v-else class="start-screen"><div class="start-content"><h1>荔湾<span class="divider">·</span>四日轮回</h1><p>荔湾广场之下，时间如莫比乌斯环般扭曲</p><button class="start-btn" @click="store.init()">踏入循环</button></div></div>
    </div>
    ...
    <div v-if="store.cycleEvent" class="cycle-toast"><span class="cycle-icon">⟳</span> {{ store.cycleEvent.cycle_count }}th cycle</div>
  </div>
</template>
...
// ── Narration typewriter ──
const T = 25; const displayedText = ref(''); const isTyping = ref(false); let tt: ReturnType<typeof setInterval>|null = null
...
function startTyping() {
  ...
  tt = setInterval(() => { i++; if (i >= raw.length) { displayedText.value = md2html(raw); isTyping.value = false; if (tt) clearInterval(tt) } else { displayedText.value = md2html(raw.slice(0,i)) } }, T)
}

function skipTyping() {
  if (!isTyping.value) return
  if (tt) clearInterval(tt)
  displayedText.value = md2html(store.currentNode?.content ?? '')
  isTyping.value = false
}

// ── Dialogue ──
const dialogLines = ref<Array<{speaker:string;text:string}>>([])
const dialogIdx = ref(0)
const visibleLines = computed(() => dialogLines.value.slice(0, dialogIdx.value + 1))
function advanceDialogue() { if (dialogIdx.value < dialogLines.value.length - 1) { dialogIdx.value++; scrollDown() } }

// ── Show choices? ──
const showChoices = computed(() => {
  if (!store.currentNode) return false
  if (isTyping.value) return false
  if (dialogLines.value.length > 0 && dialogIdx.value < dialogLines.value.length - 1) return false
  return store.choices.length > 0
})

// ── Node change ──
let prevNid = ''
watch(() => store.currentNode?.id, (nid) => {
  if (!nid || nid === prevNid) return
  prevNid = nid
  dialogLines.value = store.currentNode?.dialogue_lines ?? []
  dialogIdx.value = 0
  startTyping()
})
...
async function handleChoice(c: any) {
  ...
  if (dialogLines.value.length > 0 && dialogIdx.value < dialogLines.value.length - 1) return
  const prevN = store.currentNode?.id
  ...
  if (newN && newN !== prevN) {
    ...
    setTimeout(() => { startTyping(); scrollDown() }, 500)
    return
  }
  if (store.transitionText) {
    if (store.currentFrame) store.currentFrame = { ...store.currentFrame, transition_text: undefined }
  }
  scrollDown()
}
...
function onGlobalClick() {
  ...
  if (dialogLines.value.length > 0 && dialogIdx.value < dialogLines.value.length - 1) { advanceDialogue(); return }
}
...
async function doSave(){if(!store.currentState)return;const n=prompt('Save name:');if(!n)return;try{await axios.post('/api/saves?name='+encodeURIComponent(n),store.currentState);alert('OK')}catch{alert('FAIL')}}
async function doLoad(sid:string){try{await axios.get('/api/saves/load/'+sid);await (await import('@/api/game')).startGame();showLoad.value=false}catch{alert('FAIL')}}
async function doDelete(sid:string){if(!confirm('Delete?'))return;try{await axios.delete('/api/saves/'+sid);refreshSaves()}catch{}}
...
.chat-row { display:flex; align-items:flex-start; gap:.6rem; margin-bottom:.8rem; animation:bubbleIn .35s ease-out both;
...
.chat-bubble { flex:1; background:rgba($bg-void,.6); border:1px solid rgba($accent-gold,.08); border-radius:6px; padding:.6rem .9rem; max-width:85%; }
...
.trans-inline { margin:.8rem 0; animation:fadeIn .3s ease-out; }
